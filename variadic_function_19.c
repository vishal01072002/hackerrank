...
int sum(int count, ...)
{
    //we store all no. in list a which are count(eg 7) int counting
    va_list a; // create a list
    int sum = 0;
    va_start(a, count); //start variadic func with giving list with no. of element
    for (int i = 0; i < count; i++)
    {
        int n = va_arg(a, int); //store argument from list in n which datatype is int
        // when we call it it always move on next argument
        sum = sum + n;
    }
    va_end(a); //list a is end
    return sum;
}

int min(int count, ...)
{
    int num, min = MAX_ELEMENT; //take the smallest value is MAX
    va_list a;
    int sum = 0;
    va_start(a, count);
    for (int i = 0; i < count; i++)
...
int max(int count, ...)
{
    int num, max = MIN_ELEMENT; //take the smallest value is MAX
    va_list a;
    int sum = 0;
    va_start(a, count);
    for (int i = 0; i < count; i++)
